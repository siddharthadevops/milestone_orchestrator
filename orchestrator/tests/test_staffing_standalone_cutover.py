...
class StandaloneCutoverTestCase(unittest.TestCase):
    ...
    def persist_records(self, records):
        ...
        for record in records:
            ...
            if current["exists?"]:
                document = dict(current["value"], record=record)
            else:
                document = store._document(record, st.now_iso())
            store._store.put(key, document)
...
class TaskCompatibilityBoundaryTest(StandaloneCutoverTestCase):
    ...
    def _a_missing_field_static_task_keeps_its_pins(self):
        ...
        self.persist_records(state["tasks"])
