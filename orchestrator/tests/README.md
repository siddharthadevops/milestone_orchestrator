...
| Bounded execution seams | `suite_checkpoint_call` 16.7s/8; `canonical_plan` 14.5s/13; `judgment_call_cutover` 25.5s/35; selected `runners` 2.7s/146 | Checkpoint pins the official command, preservation of suite changes, canonical-plan observation, fresh routed judgments, output validation, subprocess, and snapshots. |
