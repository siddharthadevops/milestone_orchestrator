...
| Review, delta review and seal | Apply the same check to machinery in the target or delta. A missing or hollow document account, or immediate result-note account, is a finding when new machinery exists; later code reviews judge the machinery itself rather than historical notes. The reviewer independently verifies every claim. Any seal worker that is actually required applies the same rubric; the existing seal predicate needs no new worker. This does not suppress a valid defect or prescribe its remedy. |
