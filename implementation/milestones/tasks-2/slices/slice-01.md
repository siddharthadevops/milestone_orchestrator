...
The optional output directory remains a destination instruction and later
becomes inherited TaskExecutor context. This slice can check that the field is
well formed, but it cannot prove authority, resolve the path, define a separate
physical Worker transport, confine an executor, or prove where effects landed.
...
| Common request | The closed request object has required `work_area`, `request`, `context`, and `reference_documents`, plus optional `output_directory`. `work_area` is a non-empty JSON object carrying already-resolved execution context; `request` is a non-empty string; `context` is any JSON-plain value and remains opaque; `reference_documents` is an ordered list of non-empty strings; and `output_directory`, when present, is a non-empty string meaning the requested destination and inherited TaskExecutor routing context. No `target_path`, artifact list, or domain kind is accepted. Slice 1 validates and detaches JSON values but makes no access, canonicalization, physical Worker-call delivery, containment, placement, or contradiction-detection claim. | `implementation/milestones/tasks-2/skeleton.md:147-162,181-186,296,305`; `orchestrator/kvstore.py:177-203`; accepted amendment B5 | touch the common shape only; do-not-resolve authority, infer effects, canonicalize a destination, add a provider transport channel, or expose Brainstorming's private target |
