...
| machinery | Add only the generic TaskExecutor contract, one frozen record per scheduling decision, task-id linkage on the existing Worker marker, Worker and Brainstorming adapters, the two-key producer map, existing path/note handoffs, one Brainstorming effect seam, shared projections, and one private Brainstorming staffing rule. Reuse the existing amendment and project-context renderers, prompt traces, standing-law selector/compiler, validated call/repair seam, durable carrier, rethink state, current review comparison/restart, accounting homes, and dispatch evidence. One guard restarts the supported unbound advanced-family review shape before dispatch; it never translates an old hash. One ephemeral milestone Worker episode snapshot appends live authority on later calls and supplies the replacing safeguard extensions; it never rewrites the order or creates a cache, authority ledger, second durable request, prompt parser/rebuilder, provider lookup, new retry/recovery policy, or direct-order refresh. The destination remains frozen executor context with no separate Worker channel or placement gate. | `implementation/milestones/tasks-2/goal.md:61-87,112-160,191-203,212-240`; `orchestrator/README.md:508-511`; accepted amendments B1–B7 |
...
| cheaper_alternative | Reuse the current scheduling boundaries, central Worker call/repair seam, amendment and project-context renderers, standing-law compiler, prompt traces, durable carriers, rethink state, existing review comparison/restart, marker/accounting homes, dispatch evidence, access checks, and note-path handoff. One ephemeral snapshot per milestone Worker episode is sufficient to append current authority and replace safeguard extensions while the admitted request, strategy, roots/destination, identity, and accounting remain frozen; one comparison guard prevents supported unbound progress from skipping a family. Fingerprint translation, a cache, authority ledger, prompt parser/rebuilder, provider-history lookup, second durable request, or alternate recovery policy would add lifecycle cost without improving the authorized boundary. Keep the separate destination limit: the caller-owned initial request already carries model-visible destination instruction, and a second destination channel would not prove placement. Existing native claims still cannot establish causal placement under full-access execution. No second selector, configuration source, generic result field, scheduler, sandbox, exhaustive effect inventory, selective rollback, freshness gate, or staffing stack is needed. | existing scheduling, prompt, validation, carrier, review, access, and accounting seams; accepted amendments B1–B7 |
| cost | Build and review cost is ten bounded slices. Migration cost is no data rewrite or backfill; operational cost is one durable record/projection per scheduling decision, two small prospective values per new slice, one task-id link on new Worker markers, one ephemeral authority read/compile per milestone Worker episode, and one branch at review entry. Maintenance reuses prompt renderers/traces, standing-law compilation, repair validation, and the existing review restart; it adds no fingerprint translation, authority cache/ledger, second request form, provider lookup, parallel destination channel, placement gate, retry, liveness, freshness, sandbox, selective rollback, or parallel accounting. Omission retains unsuitable-producer cost, undiscoverable controls, unsafe destination handling, ambiguous selection, unattributed work, Worker episodes that can ignore newly binding operator law, and an unbound advanced-family edge that cannot seal its skipped history. | `this skeleton, Planned slices, Guarantee posture, and Reuse Posture`; accepted amendments B1–B7 |
| threat_model | A caller with local or authenticated API reach can supply malformed task values or path-escaping reference/output paths; a provider can return malformed native output. Validate those shapes, inherited access, canonical destination containment, paths task machinery derives from the field, and native results. The operator, product code, resolved catalogue/configuration, caller-authored complete Worker request, and full-access TaskExecutor are trusted choices. `output_directory` is a strict request instruction but not a security or success-proof boundary: tasks add no remote auth, filesystem confinement, causal effect evidence, physical Worker transport channel, or cleanup layer. | `orchestrator/README.md:77-82,323-324,511`; `orchestrator/service.py:3480-3500,3889-3927`; `orchestrator/brainstorming_lifecycle.py:276-475`; `orchestrator/contracts.py:108-157`; accepted amendments B4 and B5 |
| enforceability | Shared validators enforce request/result/configuration shapes and destination admission. The driver-owned episode snapshot feeds both the appended authority block and replacing safeguard extensions before dispatch; the existing repair seam inherits them, physical prompt traces expose revocation, and the durable carrier bypass proves authority- and profile-free consumption. The existing review comparison restarts supported unbound advanced-family progress before dispatch, while the unchanged seal predicate requires matching evidence on every effective clean round. Frozen validation options, roots/destination, task link, and accounting remain on existing seams. Independent selection, note-path handoff, task history, staffing evidence, Brainstorming pins/effects, and totals retain their existing authorities. Slices 1–10 pin these boundaries without adding fingerprint translation, an authority ledger/cache, prompt parser/rebuilder, direct-order refresh, provider lookup, alternate recovery policy, or universal placement proof. | existing validators, prompt traces, standing-law compiler, durable carriers, review restart/predicate, and task/accounting seams; accepted amendments B1–B7 |
