...
`default`) and slice 3 (the session store, the resolver, and the two live
document reads over a session — a role's assigned seats and the split-family
projection). It has no functional dependency on slice 1.
...
plus one run state key, one launch field and one read-only list route.
...
| The review cycle is the document's seats | The live seat reader `orchestrator/staffing.py:2056-2072` feeding the seal predicate's existing families parameter `orchestrator/state.py:1112-1116`. | A three-seat and a one-seat document are asserted to review and seal accordingly. |
