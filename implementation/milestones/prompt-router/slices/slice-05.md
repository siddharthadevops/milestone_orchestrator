...
edit rejects that call and restores the repository to exactly how it stood
before that physical call.
...
- **Strict — canonical-plan admission.** A successful skeleton draft is not
  ...
  and restores its exact pre-call repository snapshot.
...
active safeguards, each plan delta class, dirty pre-call repository state, and
...
when the pre-call snapshot and validated anchor are trustworthy. The reviewed
...
checks, and Git snapshot/restore primitives. The direct author adapter consumes
...
It adds fresh prompt reads and repository snapshots only when a call is actually
dispatched. That build, test, and per-call cost is lower than the omission cost
of wrong instructions or unrecoverable plan drift; caches, services, retries,
migrations, and parallel stores would add lifecycle cost without an authorised
consumer.
...
| verification | Eight checks exercise the charge matrix, exact continuation scope and custom/default meter agreement, edit-between-attempt freshness, exact prompt traces, every author/status/section/question case, active project safeguards, first-plan establishment, pre-call drift, all post-call delta classes with exact restoration, complete control absence, retained plan/task/standalone behavior, and applicable existing end-to-end behavior. The focused command names every touched test family; the official full suite remains the scheduled checkpoint. | Verification Contract above; current scope projection tests `orchestrator/tests/test_driver_implementation_size.py:2272-2371`; current freshness/fallback tests `orchestrator/tests/test_prompt_router.py:500-658`; current contract field tests `orchestrator/tests/test_prompt_contracts.py:657-700`; current anchor/guard tests `orchestrator/tests/test_canonical_plan.py:240-368`; official suite `orchestrator/README.md:544-546` |
| reuse_posture | Affected parties are operators and authors; wrong instructions, replies, scope, limits, or plan projection can waste an irreversible call or order wrong work, while exact repository rollback is locally reversible. Every granted root was searched and the relevant local/runtime seams were checked; no parallel fitting boundary exists. Reused are the prompt selector/router/corpus, existing implementation-scope projection and effective meter configuration, contract registry, canonical validator/anchor/guard, worker/task seams, project checks, trace recorder, and Git snapshots. Cheapest sufficient is one author adapter plus post-call boundary and deletion of writers; no second scope record or size policy, cache, service, store, migration, notification, or new retry is justified. | outcome authority `implementation/milestones/prompt-router/skeleton.md:103-116,125`; prompt reuse `orchestrator/prompt_sets.py:496-569`, `orchestrator/prompt_router.py:395-415`; scope/meter reuse `orchestrator/state.py:649-687`, `orchestrator/driver.py:1696-1735`; contract/plan reuse `orchestrator/prompt_contracts.py:733-805`, `orchestrator/canonical_plan.py:251-389`; repository reuse `orchestrator/gitops.py:768-824,912-981` |
| enforceability | Each asserted invariant has an expressible seam: closed charge routing and fresh whole-rung selection; declaration-driven rendering and prompt-bound registered validation; existing compiled project checks; exact prompt recording; exact canonical-block extraction/anchor/guard; full repository snapshot/restore; service/panel route absence tests; and existing task/standalone contracts. No semantic prompt policing, cross-call consistency, automatic drift restoration, plan diff/wipe, range reconciliation, optimistic/eventual delivery, or legacy migration guarantee is asserted. | routes/resolution `orchestrator/prompt_router.py:28-140,395-415`; contracts `orchestrator/prompt_contracts.py:576-805`; project checks `orchestrator/verifiers.py:398-563`; trace `orchestrator/runners.py:1663-1676,1754-1757`; plan `orchestrator/canonical_plan.py:99-133,251-389`; restore `orchestrator/gitops.py:768-824,912-981` |
...
| No owned call dispatches over drift, and each physical call either preserves a valid block outcome or restores its exact starting repository | Pre-dispatch comparison is at `orchestrator/canonical_plan.py:341-389`; refs, HEAD, index/worktree trees, and stash snapshots plus exact restore are expressible at `orchestrator/gitops.py:768-824,912-981`; byte-exact committed reads are at `orchestrator/gitops.py:1121-1140`. | Seed dirty tracked/untracked/index/stash/ref state, then exercise unchanged, valid-changed, malformed, missing, and duplicate blocks across first and later physical attempts; compare the complete snapshot and anchor/projection after each. |
