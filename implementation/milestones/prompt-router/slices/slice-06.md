...
These calls now share the skeleton's source-neutral plan boundary. A fixer may
edit ordinary work and the one canonical plan block; an invalid fixer plan edit
rejects the call and restores the governed pre-call repository boundary. A
reviewer, incremental reviewer, or rater remains report-only and receives an
explicit instruction to leave the work tree, index, and HEAD unchanged. That
instruction is trusted: the driver performs no judgment-specific mutation check
or restoration after those calls.

The canonical-block observer still runs after every physical call. An unchanged
block needs no action, a valid change follows the ordinary plan path, and an
invalid change after a trusted report-only judgment causes terminal failure
before adopting the result without judgment-specific restoration, correction,
or retry. This is a normal-path repository boundary, not protection from
arbitrary Git surgery. Damage outside the declared boundary may require the
operator to recover or restart the milestone.
...
No worker invokes another LLM. The driver-owned `reclassify` call is the only
independent finding rating; fixers decide rejection from their own verified
validity account.
...
- **Trusted report-only judgments with source-neutral plan observation.** Review,
  incremental review, and rating prompts require an unchanged work tree, index,
  and HEAD, and that instruction is trusted. The driver performs no
  judgment-specific mutation policing, restoration, block-only reinsertion,
  result invalidation, or rerun. The source-neutral observer consumes an
  unchanged block normally, sends one valid change through the ordinary plan
  path, and turns an invalid change into terminal failure before result adoption
  without judgment-specific restoration, correction, or retry.
...
- No worker-owned model dispatch or transcript/result contract for one.
...
question enforcement, editable-fixer plan outcomes, trusted report-only
source-neutral plan observation, reply-plan and suite-command
retirement, and unchanged review/fix/driver-owned-rating lifecycle behavior.
...
transport can retire, and trusted report-only plan observation needs end-to-end
...
fixer suite command; adding judgment-specific mutation policing to a trusted
report-only call; failing to send its valid plan change through the ordinary
source-neutral path; adopting its result after an invalid block; suppressing a
valid fixer plan edit because its reply is malformed; retrying an invalid fixer
plan boundary; losing the
parent review's accounting around a nested rating; accidentally exposing a
worker-owned model-dispatch path; or continuing to fixing after a rating remains
...
anchoring, and proportional work-tree/index/HEAD restoration for editing calls.
Trusted report-only judgments reuse the canonical observer without a
judgment-specific restore path. The existing review/fix/debt/seal state
machine and driver-owned classifier remain the consumers; none needs a
replacement.

The cheapest sufficient option is one shared judgment packaging seam composed
with the existing physical-call, project-extension, and source-neutral plan
primitives. Documentation or configuration alone cannot make prompt freshness,
merged reply enforcement, or plan observation enforceable. The remaining build
and maintenance cost is a small adapter and focused consumer tests, with fresh
file reads and ordinary Git operations only when an editing call requires them.
...
**Adopts** the reviewed skeleton, operator amendment A8, and the converted judgment corpus as the route,
...
original goal is used only for the charge/material examples and required job
payloads.
...
| Trusted report-only judgment boundary | `review_round`, `delta_review`, and `reclassify` prompts require an unchanged work tree, index, and HEAD, and that instruction is trusted. Capture the ordinary pre-call A once and observe only the canonical block after the call; perform no judgment-specific post-call mutation policing. An unchanged block consumes the judgment; one valid change anchors and projects through the source-neutral transition; an invalid change causes A4 terminal failure without judgment-specific restoration, correction, or retry. Slice 06 creates no preservation, restart, fingerprint-rebinding, rating, fix, rethink, or scheduling guarantee for plan-changing judgments; Slices 10–11 own those later consequences. | accepted amendment B3 as narrowed by operator amendment A8; `implementation/milestones/prompt-router/skeleton.md`, Guarantee posture | touch only shared pre-call capture plus source-neutral judgment plan completion; do-not-add a judgment-specific repository boundary or pull plan accounting/reconciliation forward |
| Routed reply vocabulary and retirements | Review and delta use the registered review result/blocked/rethink sections; fixer uses registered fix result/blocked/rethink sections and must cover exactly the queued findings; rating is `status: ok`, `kind: reclassify`, `drift_risk` and `drift_damage` each one of `low`, `medium`, `high`, or `xhigh`, plus non-empty `reason`. Every mounted question id has exactly one non-empty answer of at most 300 characters. Active project-extension fields are admitted only through the merged contract and are not unexpected fields on the otherwise closed rating envelope; collisions with routed protocol fields fail before dispatch. After the ordinary registered correction, a still-missing, malformed, or contract-inconsistent rating is a terminal failure: only started-attempt accounting survives, and the parent review cannot advance into fixing or debt. Judgment replies reject `slices`, `suite_command`, `suite_command_finding_id`, `design_update`, `design_correction`, `brainstorming_application`, planning controls, and other status-incompatible protocol fields. | `implementation/milestones/prompt-router/skeleton.md:158-160`; operator amendment A4; `implementation/milestones/prompt-router/goal.md:93-145,294-310`; standing project law; `orchestrator/prompt_contracts.py:227-388,584-620,630-736,754-833`; merged contract `orchestrator/verifiers.py:486-565`; `orchestrator/contracts.py:49-67,1251-1262` | touch routed and merged validators and remove judgment-side adoption of retired fields; do-not-weaken project safeguards, queued coverage, contests/adjudication checks, question presence/length, or status-specific envelopes |
| Retained lifecycle and driver-owned classification | Accepted outputs continue through the existing review-family cycle, delta-to-fix loop, per-finding rating/debt decision, task accounting, seals, and `need_rethink` handoff. Eligible findings from both full and delta reviews use the driver-owned classifier; fixers never launch another model. Suite checkpoints, sessions, plan recovery, and activation retain their later owners. | `implementation/milestones/prompt-router/skeleton.md:49-53,137-145,161-165`; `implementation/milestones/prompt-router/goal.md:323-333`; live consumers `orchestrator/driver.py` | touch prompt/plan boundaries around these consumers; do-not-redesign their sequencing, debt threshold, checkpoint, session, reconciliation, or seal semantics |

### Verification Contract

Focused command:

`python3 -m unittest orchestrator.tests.test_judgment_call_cutover orchestrator.tests.test_prompt_router orchestrator.tests.test_prompt_contracts orchestrator.tests.test_canonical_plan orchestrator.tests.test_p3_debt orchestrator.tests.test_producer_selection orchestrator.tests.test_author_call_cutover orchestrator.tests.test_state orchestrator.tests.test_service_api.AmendmentsApiTest`
...
| Trusted judgments use only source-neutral plan observation | `test_trusted_review_keeps_other_mutations_and_projects_plan`, `test_unchanged_trusted_review_captures_a_once_before_dispatch`, and canonical-plan counterparts | Review, delta, and rating cases treat prompt prose as trusted authoring data rather than runtime schema. Every direct call captures A once; unchanged observation performs no second work-tree snapshot, a valid block change anchors/projects, and an invalid change records terminal failure without judgment-specific restore/correction/retry. No test invents lifecycle preservation after a trusted judgment mutates the plan or arbitrary bytes. | strict within Slice 06; downstream consequences owned by Slices 10–11 |
| Routed outputs have one authority | new `test_judgment_replies_reject_legacy_plan_suite_and_design_fields` plus existing contract tests | Served review/fix/reclassify status matrices validate; every mounted question appears once, 300 characters passes and 301 fails; queued ids/severities, contests, adjudication refs, prevention paths, and completion-status project-extension fields remain enforced; no worker-owned consultation or retry envelope exists; every retired reply-plan, fixer-suite, and planning/design field fails before state adoption. | strict |
...
| Existing ordinary judgment lifecycle remains coherent | `test_fixer_commit_survives_and_is_reviewed_from_pre_fix_revision` plus current debt, producer, author, and state regressions | Clean reviews advance; ordinary findings enter fix/debt paths; delta uses its explicit pre-fix base; ratings remain graded input; prompt fallback/accounting survives. Session continuations, suite/checkpoint repair, structural plan accounting, and global legacy retirement retain their named later owners and are not closure conditions for this intermediate cut. | strict for unchanged-plan judgment lifecycle |
...
|---|---|---|
| consumers_touched | **Verified touched:** the full-review, delta-review, fixer, and nested per-finding rating consumers; direct Worker dispatch and prompt traces; prompt router and registered-plus-project contracts; mutable-amendment and standing-project-law consumption; canonical plan boundary/projection; and focused fixtures. **Verified retained:** review/fix/debt/task/seal sequencing and driver-owned classification. **Verified untouched:** Brainstorming seats, suite checkpoint, plan diff/reconciliation, schema activation, staffing/model policy, panel projection, and the bytes in all granted read-only roots. | consumers `orchestrator/driver.py`; project authority and runner boundaries above |
| pinned_facts | The exact direct jobs and material source; per-attempt fresh prompt/current amendment/scoped project extensions and granted roots/bound contract/exact trace; terminal invalid-source or invalid-policy behavior; editable fixer outcomes and trusted source-neutral judgment plan observation; exact routed reply vocabulary; no worker-owned model dispatch; and classification of eligible full/delta findings are the facts for which any deviation is a bug. | `implementation/milestones/prompt-router/skeleton.md`; standing project law; `orchestrator/prompt_router.py` |
| verification | Named checks cover the route matrix, exact traces, current-set replacement, scoped project extensions, fixer plan deltas, trusted report-only/source-neutral plan outcomes, reply/status retirement, exhausted-rating terminal failure, no worker-owned consultation, and full/delta classification flow. Existing focused router, contract, canonical-plan, debt, adversarial, producer, and task modules remain in the command; the official full suite stays checkpoint-owned. | Verification Contract above; focused contract and lifecycle tests |
| reuse_posture | Affected parties are operators and judgment workers; bad instructions, authority, validation, or plan handling can waste a call or accept/discard the wrong work. Reuse the selected prompt set, registered validators, Worker accounting, canonical boundaries, and existing `_partition_defer_candidates`; do not add another classifier, validator, cache, store, migration, generic Git recovery, or worker-owned model router. | existing prompt, judgment, classifier, and repository seams |
| enforceability | Fresh whole-rung resolution, declaration-driven rendering, registered prompt-bound plus scoped project-extension/root validation, per-attempt completion before reply validation, exact trace persistence, strict authority parsing at preparation, source-neutral canonical-block comparison, proportional fixer restoration, and existing state transition gates can express every guarantee asserted here. Tests pin editable fixers, trusted report-only judgments, and strict-rating consumers. No judgment-specific mutation policing for trusted calls, unrelated-Git recovery, semantic prompt policing, provider-history authority, cross-call consistency, optimistic/eventual delivery, plan-history repair, or schema activation guarantee is asserted. | resolution `orchestrator/prompt_sets.py:505-578`, `orchestrator/prompt_router.py:410-438`; validation `orchestrator/prompt_contracts.py:754-833`, `orchestrator/verifiers.py:211-216,486-565`; physical boundary `orchestrator/runners.py:2918-2980,3251-3320`; plan/restore `orchestrator/canonical_plan.py:135-146,329-552`, `orchestrator/gitops.py:827-872`; state failure/transition `orchestrator/driver.py:4066-4134`; accepted amendment B3 |
...
| Trusted report-only judgments receive only source-neutral plan observation | Canonical comparison/projection is at `orchestrator/canonical_plan.py`; review/delta/rating transition consumers are at `orchestrator/driver.py:11331-11764,12058-12396,12398-12719`. Prompt-set structure and registered contracts remain the only runtime validity checks; prompt prose is trusted. | For review, delta, and rating, prove there is no prompt-content validity check and no judgment-specific mutation check, restoration, isolation, block-only reinsertion, invalidation, or rerun. Assert unchanged blocks consume normally, valid changes enter the ordinary plan path, and invalid changes enter terminal failure before result adoption without judgment-specific restore/correction/retry. |
| Judgment replies and questions obey the routed-plus-project contract and cannot re-author plan, suite, or retired design state | Question ids/answers, allowed fields, project extensions, fixer coverage, and rating scales use the existing routed validators. | Exercise question bounds, extension validation, `blocked`/`need_rethink`, and retired or status-incompatible fields. Prove `retry` and consultation fields are not part of the fixer contract. |
| Existing review/fix/delta/debt behavior survives the cutover | Current lifecycle consumers are in `orchestrator/driver.py`; fixer and rating result rules are in `orchestrator/prompt_contracts.py`. | Drive one clean cycle, one fixed finding, one direct rejected finding, one delta finding, and one deferred rating. Prove eligible findings from both full and delta reviews use the same classifier and no worker launches another model. |
