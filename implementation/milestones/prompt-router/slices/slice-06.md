...
| Trusted report-only judgment boundary | `review_round`, `delta_review`, and `reclassify` prompts require an unchanged work tree, index, and HEAD, and that instruction is trusted. Apart from source-neutral canonical-block observation, perform no post-call mutation check, restoration, isolation, block-only reinsertion, judgment invalidation, or rerun for these jobs. An unchanged block consumes the judgment; one valid change follows the ordinary plan path; an invalid change causes A4 terminal failure before result adoption without judgment-specific restoration, correction, or retry. | accepted amendment B3; `implementation/milestones/prompt-router/skeleton.md`, Guarantee posture; trusted instruction in `implementation/brainstorming/prompt-router/adapted-kinds/shared/shared.json` | touch only source-neutral plan observation around these consumers; do-not-add a judgment-specific repository boundary or weaken editing-call, seat, or checkpoint boundaries |
...
| Trusted judgments use only source-neutral plan observation | new `test_trusted_judgments_use_source_neutral_plan_observer` | Review, delta, and rating cases treat prompt prose as trusted authoring data rather than a runtime schema; the driver performs no judgment-specific mutation check, restoration, isolation, block-only reinsertion, invalidation, or rerun. An unchanged block consumes the result, one valid change follows the ordinary plan path, and an invalid change records terminal failure before result adoption without judgment-specific restore/correction/retry. | strict |
