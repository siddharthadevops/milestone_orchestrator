...
- **Strict checkpoint gate:** a failed suite checkpoint cannot be deferred or
  reclassified; no seal passes until its dedicated fixer certifies the complete
  command plan on the final bytes, or a later fresh checkpoint returns `passed`
  or `no_suite` after that proof is invalidated.
...
| Read-only seat, suite, and trusted-judgment boundaries | Ordinary governed mutation by a read-only seat or suite checkpoint is restored. A valid plan-block change is reinserted alone and committed, invalidating that seat's readiness or the checkpoint status. Failed suite evidence remains non-deferrable until its dedicated fixer returns `ok` after the full command plan passes; that proof is reused only for exact matching bytes and commands. Separately, `review_round`, `delta_review`, and `reclassify` trust their report-only prompt instruction: only the source-neutral canonical-block observer runs after them; no judgment-specific mutation policing, restoration, block-only reinsertion, invalidation, or rerun occurs. | `goal.md:149-180,187-226`; accepted amendments B1 and B3; operator ruling 2026-08-27 | touch seat/checkpoint snapshot, fixer certification reuse, and source-neutral judgment plan observation; do-not-apply the seat/checkpoint mutation boundary to trusted judgments |
