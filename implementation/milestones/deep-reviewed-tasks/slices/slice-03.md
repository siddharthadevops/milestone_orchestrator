...
rethink, using the current repository state. No agreement or an operationally
failed discussion fails the originating work. A terminal task is never reopened.
...
- **Strict:** every offered reviewed-work call uses its exact Prompt Router
  ...
  work; and rethink success or failure has the stated originating-work outcome.
...
remaining open while waiting, re-entering the same phase after agreement, and
failing without reopening a terminal record after discussion failure. Existing
default-lifecycle, route, Brainstorming-producer, and size-isolation goldens must
remain satisfied.
...
resuming a different phase, or accidentally changing explicitly ordered tasks.
Route spies, exact history assertions, restart cases, and accounting comparisons
...
| Rethink continuity and terminality | `need_rethink` leaves the originating reviewed work in progress and durably attaches one ordinary repository-backed Brainstorming. Agreement returns that same work to the interrupted semantic phase from current repository state. No agreement, missing/lost session, or operational terminal failure fails the origin. Neither path reopens a terminal task record. Existing accepted-plan/reconciliation consequences remain in force. | mandate `implementation/milestones/deep-reviewed-tasks/goal.md:101-105`; milestone pin `implementation/milestones/deep-reviewed-tasks/skeleton.md:51-53,131`; durable wait and handoff `orchestrator/driver.py:6680-6722,7807-8003`; repository session `orchestrator/brainstorming_milestone.py:269-340,417-421` | touch origin terminalization and phase re-entry; do-not-return `need_rethink` as a public success/failure result, admit a successor reviewed task, reopen a terminal id, or bypass existing design-repair/reconciliation law |
| Slice boundary and direct-task isolation | Slice 3 adds no public TaskExecutor id, route, panel field, or public error code. The catalogue remains exactly `agent_call` and `brainstorming` here. Direct orders retain their task records and neither direct executor gains size control; reviewed implementation size control remains applicable only when its selected producer is `agent_call`. | slice allocation `implementation/milestones/deep-reviewed-tasks/skeleton.md:41-53,108-110`; current catalogue `orchestrator/tasks.py:66-125`; direct dispatch `orchestrator/task_api.py:580-590` | touch no public ordering surface; do-not-publish Slice 4/5 results or task types early, add a private milestone/Agent99 route, or add Brainstorming size machinery |

### Verification Contract

Focused command:

`python3 -m unittest orchestrator.tests.test_reviewed_call_routing orchestrator.tests.test_reviewed_lifecycle orchestrator.tests.test_reviewed_policy orchestrator.tests.test_author_call_cutover orchestrator.tests.test_judgment_call_cutover orchestrator.tests.test_session_call_cutover orchestrator.tests.test_session_repository_seal orchestrator.tests.test_worker_tasks orchestrator.tests.test_task_activity orchestrator.tests.test_brainstorming_slice_production orchestrator.tests.test_driver_implementation_size orchestrator.tests.test_tasks orchestrator.tests.test_task_api`
...
| Successful rethink continues the same origin and phase | new `ReviewedCallRoutingTest.test_rethink_success_reenters_each_eligible_phase_on_the_same_origin`; retained `RepositorySealTest.test_repository_rethink_reenters_fresh_without_application` | Each eligible origin stays non-terminal while waiting; accepted repository state causes a fresh call of that same semantic phase under the same reviewed-work origin, without an internal successor task. | strict outcome; best-effort session delivery |
...
| consumers_touched | Verified direct consumers are the reviewed-work lifecycle and its milestone driver caller. Unit/round/event accounting and milestone task projections consume the changed evidence identity. The standalone task host is a retained comparison, not a changed consumer. Searches across Life, Agent99, life_product_components, and Tutor found no code consumer of these task ids or route names, so no product adapter is added. | lifecycle caller `orchestrator/driver.py:8251-8263`; evidence readers `orchestrator/state.py:2335-2458`; milestone task projection `orchestrator/service.py:4343-4376,4447-4458`; direct host `orchestrator/task_api.py:580-590` |
...
| pinned_facts | The seven hard rows pin only deviations that alter behavior or a cross-slice contract: exact offered routes, sole prompt authority, call-versus-task identity, retained evidence/accounting, exact rethink vocabulary, rethink terminality, and the public/direct-task boundary. No internal class name, storage layout, control-flow order, or new error vocabulary is made contractual. | this note, `Pinned-Facts Table`; slice allocation `implementation/milestones/deep-reviewed-tasks/skeleton.md:108-110`; prompt/rethink pins `implementation/milestones/deep-reviewed-tasks/skeleton.md:130-131` |
| verification | Six executable rows combine one new lifecycle integration matrix with retained direct-author, judgment, session-seat, repository-rethink, explicit-task, Brainstorming-size, and default-lifecycle goldens. The focused command pins this slice; checkpoint and extended suites remain repository gates and cannot be claimed without execution. | current route proofs `orchestrator/tests/test_author_call_cutover.py:673-677`; `orchestrator/tests/test_judgment_call_cutover.py:234-248`; `orchestrator/tests/test_session_call_cutover.py:148-192`; rethink proof `orchestrator/tests/test_session_repository_seal.py:437-485`; suite authority `orchestrator/README.md:565-586` |
| enforceability | Prompt Router plus the three bound adapters enforce route/prompt authority. Existing optional task attribution, draft/round/events, crash marker, and aggregation can enforce evidence-only single counting. Repository Brainstorming wait and sealed-range handoff can enforce same-phase continuation. Two current gaps are implementation gates rather than promises: reviewed calls still always admit an `agent_call` task, and `_start_rethink` terminalizes that call before attaching the discussion. Direct-task isolation is enforceable because the standalone host is separate. Physical uniqueness remains unenforceable and therefore best-effort under the stated at-least-once law. | route mechanisms `orchestrator/prompt_router.py:77-127,424-460`; evidence mechanisms `orchestrator/state.py:858-941,1060-1097,2335-2458`; gaps `orchestrator/driver.py:3973-4089,6553-6565`; continuation `orchestrator/driver.py:6680-6722,7932-8003`; delivery limit `orchestrator/state.py:1-17` |
