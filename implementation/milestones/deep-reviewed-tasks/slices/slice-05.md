...
| Identity, calls, rethink, and terminal result | One acknowledged outer task id exclusively and durably locates one reviewed lifecycle. Every later id admitted over the same work area starts a fresh production and review history: no terminal, stopped, or forgotten earlier id can supply its seal, gate commit, or accounting. Production, review, fix, delta-review, classification, and rethink calls are evidence, never child `agent_call` tasks. `need_rethink` keeps that task open. Agreement resumes the interrupted semantic phase from the accepted repository state under the same standalone task, without requiring a canonical-plan anchor or opening milestone plan reconciliation; failed Brainstorming fails it. Success is the existing generic envelope whose native result is exactly `production_result`, `review_evidence` (`seal_attempt`, `reviews`, `verification_event_seq`), and `gate_commit`; accounting derives once from that task's lifecycle evidence. The task record mutates only from `result: null` to terminal `success` or `failure`. | task ownership and rethink outcome `implementation/milestones/deep-reviewed-tasks/goal.md:36-40,75-79,101-105`; caller boundary `implementation/milestones/deep-reviewed-tasks/skeleton.md:48-56,134,138`; result projection `orchestrator/driver.py:590-634`; terminal envelope/mutation `orchestrator/tasks.py:822-837,1018-1092` | touch the standalone adapter, lifecycle association, and result handoff; do-not-add child tasks, a second result/evidence store, expose `need_rethink`, require milestone plan state for standalone continuation, reuse another outer id's evidence, flatten the native result, double-count calls, or reopen a terminal id |
...
| Rethink agreement resumes standalone work without milestone state | new `ReviewedTaskOrderingTest.test_rethink_agreement_resumes_same_standalone_phase_without_plan_reconciliation` | A standalone reviewed phase that returns `need_rethink` stays open; successful repository-backed Brainstorming returns that same task to the interrupted phase from the accepted repository state, without a canonical-plan anchor, reconciliation record, replacement task, or terminal intermediate result. | strict outcome; best-effort physical uniqueness |
...
| One public id owns one reviewed lifecycle and one terminal result | Standalone record CAS at `orchestrator/task_api.py:128-184`; null-to-terminal mutation at `orchestrator/tasks.py:822-837`; append-only lifecycle state at `orchestrator/state.py:1-17`. | **Current gaps:** an admitted standalone id has no exclusive reviewed-lifecycle association, and the successful rethink handoff requires milestone plan state. Acceptance requires the same id and evidence across every named restart window; successful standalone rethink resumes its interrupted phase without plan reconciliation, replacement admission, or an intermediate terminal result; every later id on the released work area must produce, review, gate, and account from fresh lifecycle evidence. |
