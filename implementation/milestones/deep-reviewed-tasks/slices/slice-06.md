...
- **Strict:** a valid deep order durably resolves both child policies before
  ...
  before releasing the work area; old records retain their original meaning.
...
- No implementation child, implementation part, size-cut continuation, deep
  success from documentation completion, parent accounting aggregate, or
  aggregate commit. Slice 7 owns those outcomes; this slice is not a release
  boundary by itself. Operator Stop retains the generic terminal-failure path.
...
terminal child remains immutable. Interrupt delivery to an in-flight process
remains best-effort, but its late return cannot produce child or parent success.
...
| Parent Stop | `POST /api/tasks/<deep-id>/stop` retains the generic standalone contract. An accepted Stop returns `stopping`, prevents parent success and later child admission, and closes the parent as `failure` with the operator reason. If the documentation child is active, it also closes as `failure` under that reason before the work area is released; a terminal child is unchanged. No new lifecycle step begins after acceptance, while interruption of an already-running physical process remains best-effort. | public standalone assignment `implementation/milestones/deep-reviewed-tasks/skeleton.md:114,128`; generic Stop contract `implementation/milestones/deep-reviewed-tasks/slices/slice-05.md:139`; current route/host control `orchestrator/service.py:4263-4287`; `orchestrator/task_api.py:570-609,739-817` | touch composite Stop propagation and settlement; do-not-add a route, reopen or rewrite a terminal child, release the work area while the child remains open, turn a late return into success, or promise process-level exactly-once interruption |
...
| Parent Stop settles the active child | new `DeepTaskDocumentationTest.test_stop_parent_fails_active_documentation_child_before_releasing_work_area`; retained direct/reviewed task Stop tests | After an accepted parent Stop, the active child and parent each close once as failure with the operator reason, no new lifecycle step or implementation child starts, and the work area remains owned until the child settles. A late physical return cannot become success. | strict terminality and sequencing / best-effort interrupt delivery |
| Compatibility and scope remain bounded | new `DeepTaskDocumentationTest.test_slice_six_never_admits_implementation_or_completes_parent`; retained direct/reviewed task and old-record tests | Documentation completion creates no implementation record, size event, parent terminal result, aggregate charge, aggregate commit, route change, old-record rewrite, or changed direct-task behavior. | strict compatibility / best-effort convenience projections |
...
| enforceability | Closed order and reviewed-policy resolvers, service access/path/Git/lease checks, immutable task results, the store's registry lock plus point CAS, generic task reads, generic Stop control, reviewed-task host/recovery, and current child result can enforce most claims. Three mechanisms do not yet exist and are implementation gates: the deep schema/resolver, canonical `parent` relation with serialized uniqueness/retention, and a deep host/adoption/Stop path that admits or reuses the documentation child, settles it on parent Stop, and withholds implementation and parent success. The existing store cannot currently express semantic child uniqueness, and the host treats every non-direct/non-Brainstorming executor as reviewed work; those are gaps, not present guarantees. UI grouping, physical-call uniqueness, and interrupt delivery have no strict mechanism and remain best-effort. | existing enforcement `orchestrator/tasks.py:627-798,897-928,1047-1086`; `orchestrator/service.py:4193-4258,4263-4287,4326-4352`; `orchestrator/task_api.py:132-194,570-609,629-817`; missing catalogue id `orchestrator/tasks.py:105-274`; delivery limit `orchestrator/state.py:1-17` |
...
| Parent Stop settles the active documentation child | Generic standalone Stop route and result vocabulary at `orchestrator/service.py:4263-4287`; host Stop control and reviewed terminal selection at `orchestrator/task_api.py:570-609,739-817`; durable parent/child identity required above. | **Current gap:** no composite host exists to carry an accepted parent Stop to its active child and wait for that child to settle. Acceptance requires both records to close as failure with the operator reason before work-area release, while retaining best-effort interruption for an already-running physical process. |
