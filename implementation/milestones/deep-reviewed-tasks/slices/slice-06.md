...
declare the deep task successful. If operator Stop instead makes the parent
terminal in this slice, its failure result aggregates the settled documentation
child's accounting once; terminality cannot strand child charges outside the
parent result.

### Ownership and consumers

The deep parent owns composition—the frozen child policies, documentation-first
boundary, and durable child identity—and the child-first projection required by
any terminal parent result. The documentation child remains an ordinary
`reviewed_task`; parentage grants no different review behavior or private entry
point. The shared task service and store retain admission, visibility, access,
and persistence ownership. Prompt Router, Staffing Router, the reviewed
lifecycle, and Git retain their existing authority.
...
- **Strict:** a valid deep order durably resolves both child policies before
  ...
  by this slice; an accepted parent Stop fails the parent and any active child
  before releasing the work area, and the parent's terminal failure aggregates
  the settled child's accounting once; old records retain their original
  meaning.
- **Best-effort:** physical-call uniqueness and provider delivery retain the
  reviewed lifecycle's at-least-once limit, including interrupt delivery to an
  already-running process. Child grouping, chips, navigation, and display
  freshness remain conveniences and cannot control execution.
...
- No implementation child, implementation part, size-cut continuation, deep
  success from documentation completion, normal open-parent accounting
  aggregate, or aggregate commit. Slice 7 owns implementation delivery,
  multi-child aggregation, and deep success; this slice owns only the
  child-first accounting required if Stop creates a terminal parent. This slice
  is not a release boundary by itself.
...
The generic Stop action remains part of the public deep order. Once a Stop is
accepted for an open parent, it reports `stopping`, no later child or reviewed
lifecycle step may begin, and the parent closes as `failure` with the operator
reason. An active documentation child must take that same Stop outcome and
reach terminal failure before the composite releases the work area; an already
terminal child remains immutable. Before the parent becomes terminal, its
failure result must aggregate the settled child's duration, usage, cost, and
partiality without recording any physical charge again. Interrupt delivery to
an in-flight process remains best-effort, but its late return cannot produce
child or parent success.
...
deletion of live recovery authority, leaving a documentation child active after
its parent is stopped, or admitting implementation too early.
...
make those faults observable. Stop coverage also makes terminalization before
the child's accounting aggregate observable.
...
| Parent Stop | `POST /api/tasks/<deep-id>/stop` retains the generic standalone contract. An accepted Stop returns `stopping`, prevents parent success and later child admission, and closes the parent as `failure` with the operator reason. If the documentation child is active, it also closes as `failure` under that reason before the work area is released; a terminal child is unchanged. The parent becomes terminal only with the settled child's duration, usage, cost, and partiality aggregated once in its failure result. No new lifecycle step begins after acceptance, while interruption of an already-running physical process remains best-effort. | child-first accounting `implementation/milestones/deep-reviewed-tasks/skeleton.md:60-68,80-89,136`; public standalone assignment `implementation/milestones/deep-reviewed-tasks/skeleton.md:114,128`; generic Stop contract `implementation/milestones/deep-reviewed-tasks/slices/slice-05.md:139`; terminal result shape `orchestrator/tasks.py:1267-1341`; current route/host control `orchestrator/service.py:4263-4287`; `orchestrator/task_api.py:570-609,739-817` | touch composite Stop propagation, settlement, and terminal accounting projection; do-not-add a route or ledger, reopen or rewrite a terminal child, release the work area while the child remains open, terminalize the parent before accounting is preserved, turn a late return into success, or promise process-level exactly-once interruption |
...
| Parent Stop settles the active child and accounting | new `DeepTaskDocumentationTest.test_stop_parent_fails_active_documentation_child_before_releasing_work_area`; retained direct/reviewed task Stop tests | After an accepted parent Stop, the active child and parent each close once as failure with the operator reason, no new lifecycle step or implementation child starts, and the work area remains owned until the child settles. The terminal parent result equals the settled child's duration, usage, cost, and partiality without double counting; a late physical return cannot become success. | strict terminality, sequencing, and child-first accounting / best-effort interrupt delivery |
| Compatibility and scope remain bounded | new `DeepTaskDocumentationTest.test_slice_six_never_admits_implementation_or_completes_parent`; retained direct/reviewed task and old-record tests | Ordinary documentation completion creates no implementation record, size event, parent terminal result, open-parent accounting aggregate, aggregate commit, route change, old-record rewrite, or changed direct-task behavior. The only Slice 6 parent aggregate is the child-first accounting required by a terminal Stop failure. | strict compatibility / best-effort convenience projections |
...
| pinned_facts | The nine hard rows pin only public or cross-slice deviations that are bugs: executor/routes, two policy groups, admission/errors, child order/relation, documentation-first scope, uniqueness/recovery/retention, parent Stop, size ownership, and compatibility/projection posture. Internal class names, storage layout, polling, control flow, and UI layout remain choices. | slice allocation `implementation/milestones/deep-reviewed-tasks/skeleton.md:114-115`; milestone hard contracts `implementation/milestones/deep-reviewed-tasks/skeleton.md:128,132,135-139`; existing public names `orchestrator/tasks.py:18-35,105-274` |
| verification | Eight executable rows combine a new deep-documentation contract module with retained catalogue/panel, reviewed-policy, reviewed-result, generic record/store, API, recovery, Stop, and compatibility checks. The focused command pins this slice. Repository suites remain separate gates and are not claimed without execution. | current public reviewed proof `orchestrator/tests/test_reviewed_task_api.py:297-445`; record/CAS proof `orchestrator/tests/test_tasks.py:637-816`; panel proof `orchestrator/tests/test_task_panel.py:18-61`; suite authority `orchestrator/README.md:565-586` |
| enforceability | Closed order and reviewed-policy resolvers, service access/path/Git/lease checks, immutable task results, the store's registry lock plus point CAS, generic task reads, generic Stop control, reviewed-task host/recovery, and current child result can enforce most claims. Three mechanisms do not yet exist and are implementation gates: the deep schema/resolver, canonical `parent` relation with serialized uniqueness/retention, and a deep host/adoption/Stop path that admits or reuses the documentation child, settles it on parent Stop, projects its accounting once into the terminal parent failure, and withholds implementation and parent success. The existing store cannot currently express semantic child uniqueness, and the host treats every non-direct/non-Brainstorming executor as reviewed work; those are gaps, not present guarantees. UI grouping, physical-call uniqueness, and interrupt delivery have no strict mechanism and remain best-effort. | existing enforcement `orchestrator/tasks.py:627-798,897-928,1047-1086,1267-1341`; `orchestrator/service.py:4193-4258,4263-4287,4326-4352`; `orchestrator/task_api.py:132-194,570-609,629-817`; missing catalogue id `orchestrator/tasks.py:105-274`; delivery limit `orchestrator/state.py:1-17` |
...
| Parent Stop settles the active documentation child and accounts it | Generic standalone Stop route and result vocabulary at `orchestrator/service.py:4263-4287`; host Stop control and reviewed terminal selection at `orchestrator/task_api.py:570-609,739-817`; terminal accounting fields at `orchestrator/tasks.py:1267-1341`; durable parent/child identity required above. | **Current gap:** no composite host exists to carry an accepted parent Stop to its active child, wait for that child to settle, and form a child-first parent result. Acceptance requires both records to close as failure with the operator reason and the parent's terminal accounting to equal the settled child's accounting before work-area release, while retaining best-effort interruption for an already-running physical process. |
