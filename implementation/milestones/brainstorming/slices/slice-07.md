...
Slice 8 corrects the distinction between the unaccepted recovery baseline and
lead-accepted target state. It preserves this slice's route and target
projection: before completed lead work the target is visibly not yet accepted;
afterward the view reads only the accepted Brainstorming revision.
...
| Discussion, target, and result converge in order | `test_transcript_ballots_target_and_result_follow_accepted_state` | Successive snapshots show accepted turns and ballots in order, canonical complete Markdown, the referenced accepted target revision, and then the exact terminal result without reading an unaccepted live target; the same final target remains readable after terminalization and service reload. | strict state; eventual observation |
| The pre-lead view stays available | `test_coordination_without_lead_acceptance_is_not_yet_accepted` | After coordination starts but before any completed lead turn, the authorized view still succeeds and returns the target reference with null revision, existence, and content plus `truncated: false`; it never reads or previews the recovery baseline as accepted. | strict |
...
| verification | **focused:** seven named checks pin the routes/schema, pre-read authorization and one-revision consistency, ordered transcript/ballot/target/result convergence, the coordination-without-lead not-yet-accepted state, honest target previews, polling/stale/stop behavior, and milestone compatibility. **full:** repository unittest discovery remains the milestone gate. | `implementation/milestones/brainstorming/slices/slice-07.md:116-134`; `implementation/milestones/brainstorming/skeleton.md:124-139`; `orchestrator/README.md:321-324` |
...
- **Compatibility:** lifecycle route envelopes, transcript, target revision
  identifiers, milestone panel, and `/api/runs` remain unchanged. The corrected
  durable state keeps `accepted_target_revision` null until completed lead work,
  so Slice 8 must adapt this existing view consumer and its focused regression
  check. During that interval the view remains available, reports the target as
  not yet accepted, and never previews the recovery baseline as accepted. The
  page is a replaceable consumer of the stable view projection, not a new
  authority.
...
| Canonical ordered transcript and accepted target | Version-selected canonical transcript rendering and immutable target-revision lookup (`orchestrator/brainstorming.py:352-424,1918-2007,2030-2042`) | Transition matrices compare the projection with `chat.md`, accepted ballot order, and the target bytes named by the same state's accepted revision; a coordination state with no accepted revision returns the pinned not-yet-accepted target instead of attempting a revision read. |
