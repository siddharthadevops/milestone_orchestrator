...
Slice 8 owns the corrective integration of this envelope-repair allowance with
Slice 3's target-mutation correction allowance. Each is available once while
the same worker action is pending, and neither resets the other. The historical
Slice 2 unit is not rerun.
...
| Validation and repair | Each scheduled discussion turn accepts exactly one contract-valid JSON object. It has one envelope-repair allowance for its whole lifetime: an invalid first response permits one repair exchange in the same logical session, while another invalid response is a protocol failure. A target-mutation correction does not reset this allowance, and using this allowance does not reset the separate target-mutation correction. Invalid raw responses remain technical evidence and are never accepted as discussion. The repair exchange is control, not another completed discussion turn. | `orchestrator/runners.py:414-459,962-1053`; `implementation/milestones/brainstorming/skeleton.md`, Invalid target mutation; `implementation/milestones/brainstorming/goal.md:174-187,230-241` | touch reusable validation/repair entry point and Slice 8's persisted cross-check; do-not-parse prose, accept multiple objects, reset either allowance for the same pending worker action, or create a fresh repair session |
