...
Slice 8 owns the corrective regression coverage for the request boundary: every
positive integer remains a valid caller-selected `max_rounds`, including values
above 16. It also updates the shared state projection so session creation
records an unaccepted recovery baseline while accepted target state stays null
until completed lead work. The historical Slice 1 unit is not rerun, and no
quota-version, migration, or compatibility branch is introduced.
...
|---|---|---|---|
| Request and context | Required request names are `workspace_path`, `target_path`, `question`, `context`, and `max_rounds`. The three text fields are non-empty strings and `max_rounds` is any positive integer chosen by the caller; this milestone adds no fixed upper bound. `context` contains required non-empty string `brief`, optional ordered list of non-empty string `references`, and optional JSON-compatible `source_payload`. `source_payload` is preserved unchanged and never drives core state. `target_path` is the output; consulted material is context. | `implementation/milestones/brainstorming/skeleton.md`, Request and context; `implementation/milestones/brainstorming/goal.md:65-115` | touch product-neutral validation; do-not-add answer options, a problem/domain type, caller interpretation, or a fixed global round policy |
...
|---|---|---|---|
| Request/context validation is closed and side-effect free | `test_request_and_context_contract` | Positive round bounds including 1, 16, and a value above 16 are accepted. Missing/wrong fields, empty required text, a non-positive or non-integer round bound, non-string-list `references`, non-JSON `source_payload`, or forbidden taxonomy/answer fields are rejected without creating state. | strict |
...
| pinned_facts | **closed facts:** request/context names and opacity; caller-selected positive round bounds with no fixed global upper limit; resolved roster invariants and fallback record; two closure-policy values; lifecycle states `created`/`running`/`success`/`failure` and their legal transitions; independent atomic/auditable state; retained terminal evidence; no milestone vocabulary or later-slice behavior. | this note, Pinned-Facts Table; `implementation/milestones/brainstorming/skeleton.md`, Pinned Facts; `implementation/milestones/brainstorming/goal.md:50-115,266-279` |
| verification | **focused:** the named checks cover request admission, opacity, roster/policy resolution, immutability, append-only history, atomic/stale writes, result coherence, and ledger independence. **full:** the repository unittest discovery command remains the milestone gate. | this note, Verification Contract; `implementation/milestones/brainstorming/goal.md:336-374`; `implementation/milestones/brainstorming/skeleton.md`, Verification Contract |
...
| enforceability | **shape/closed vocabularies:** executable validators accept every positive integer and reject only non-positive or non-integer round bounds. **opaque JSON:** canonical JSON validation plus equality/metamorphic tests. **whole durable updates/concurrency:** existing locked atomic whole-value CAS. **immutability/history:** extend the existing prefix-rewrite guard to the product-neutral state. **lifecycle/result representation:** extend the existing explicit-transition guard; terminal fields are write-once. **excluded:** no semantic `success`, liveness, delivery, API, resource quota, or view guarantee is asserted here; Slice 5 owns success eligibility. | this note, Verification Contract and Enforceability Gate; `implementation/milestones/brainstorming/skeleton.md`, Guarantee Posture |
...
- **Compatibility:** the new seam consumes JSON-compatible caller context,
  ...
  runtime integration. Slice 8 adds the baseline-versus-acceptance correction
  without changing this request contract. Later slices can consume it without
  changing current milestone contracts or routes.
...
| Caller chooses the round bound | The request validator accepts any positive integer and has no milestone-defined maximum | Focused tests accept a value above 16 and reject only non-positive or non-integer values; no contract-version or migration branch appears. |
