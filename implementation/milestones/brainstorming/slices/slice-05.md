...
Slice 8 owns the corrective integration of Slice 3's target-mutation correction
with this slice's existing failure and closing contract. A second invalid
target mutation before the same worker action completes restores only the
target and publishes one coherent failure, without accepting the rejected
work. It preserves the complete terminal state, closing, result, revision
identities, and final accepted target; the historical Slice 5 unit is not
rerun.
...
- No VCS or repository dependency in closure's target-revision validation or
  recovery, no concurrent-external-write attribution, and no modification or
  recovery of any path other than `target_path`. Participant contextual
  inspection remains inherited.
...
invalidates all votes gathered by that attempt. The same pending closure worker
action may be attempted once after recovery. A second invalid target mutation
before it completes restores the target again and returns coherent `failure`;
the rejected work accepts no ballot, turn, revision, or round. This allowance
is independent of the one discussion-envelope repair and neither resets the
other.
...
|---|---|---|---|
| Closure boundary | Closure is eligible only after a complete persisted roster pass, which includes one completed lead turn, and a non-null `accepted_target_revision` created by completed lead work. The lead may propose closure against that revision; the proposal is the lead's `accept` vote. Without a lead proposal there is no accepted ballot. Ballot activity is control work, consumes no turn or round, and at most one ballot is accepted for a completed round. | `implementation/milestones/brainstorming/skeleton.md:25-28,75,77,103,105`; `implementation/milestones/brainstorming/goal.md:230-259`; Operator Amendment A1, **Target versioning clarification**; `orchestrator/brainstorming.py:654-737,1299-1341` | touch closure eligibility over accepted progress; do-not-close against the recovery baseline, change discussion order, round accounting, or interpret Markdown |
...
| Target-revision validity | Every vote in one accepted ballot refers to the same current Brainstorming revision. Any target mutation during proposal, vote collection, or ballot acceptance invalidates every vote from that attempt, accepts no ballot/result/round change, and recovers only `target_path` from the last accepted Brainstorming revision. The pending worker action has one target-mutation correction allowance; repetition before it completes restores the target and terminalizes coherent `failure`. The independent envelope-repair allowance cannot reset it. A later completed lead edit makes an older ballot ineligible for success. External concurrent target writes are outside the run contract. | `implementation/milestones/brainstorming/skeleton.md`, Invalid target mutation; Operator Amendment A1, **Target versioning clarification**; `orchestrator/brainstorming_coordination.py:137-153,583-690,731-877` | touch the existing target-exclusive acceptance, bounded correction, failure, and target-only recovery seams; do-not-let closure target-version validation or recovery inspect Git/VCS state or repository metadata, and do-not-recover another path |
...
| Success is revision-bound and atomic | `test_success_requires_current_approved_ballot_and_is_atomic` | Success without a ballot, without a lead-accepted target revision, or with a failed/stale ballot, recovery-baseline revision, wrong round/revision, incomplete summary, or mismatched references is rejected; a current approving ballot exposes ballot, success result, closing, and transcript together, with every terminal object vote present in unresolved objections. | strict |
...
| pinned_facts | **closed facts:** complete-round lead-proposed ballots; exact accepted-ballot shape; current-revision binding and target-only invalidation recovery; one bounded target-mutation correction and coherent failure independent of envelope repair; the two exact policy decisions; one ballot per round; discussion-after-failure and failure-at-exhaustion; ballot-gated atomic success; coherent explicit failure/result/closing; and no public or later-slice surface. | this note, Pinned-Facts Table; `implementation/milestones/brainstorming/skeleton.md`, Invalid target mutation; Operator Amendment A1, **Target versioning clarification** |
| verification | **focused:** the closure checks plus Slice 8's corrective regression pin ballot shape/truth, both policies, target invalidation/recovery, bounded repeated-mutation failure, real discussion between ballots, exhaustion, success eligibility/atomicity, zero-turn failure, contention, and compatibility. **full:** repository unittest discovery remains the milestone gate. | this note, Verification Contract; `implementation/milestones/brainstorming/skeleton.md`, Verification Contract; `implementation/milestones/brainstorming/goal.md:336-374` |
